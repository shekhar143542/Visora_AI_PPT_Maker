...
export async function parsePowerPointFileBasic(filePath: string): Promise<ParsedSlide[]> {
  try {
    console.log('📄 Starting basic PowerPoint parsing for:', filePath);
    ...
    for (let i = 0; i < slideRefs.length; i++) {
      ...
      try {
        ...
        if (slideXml) {
          ...
          const slide = await parseSlideBasic(slideDoc, i + 1);
          slides.push(slide);
...
async function parseSlideBasic(slideDoc: Document, slideNumber: number): Promise<ParsedSlide> {
  console.log(`📄 Parsing slide ${slideNumber} content...`);
  
  // Extract text using a more robust approach
  let allText = '';
  
  // Method 1: Look for a:t elements (most common in PowerPoint)
  ...
  // Method 2: If no text found, look for any element with text content
  if (!allText.trim()) {
    console.log('📄 No a:t text found, looking for any text content...');
    ...
    for (let i = 0; i < allElements.length; i++) {
      ...
      if (element.textContent && element.textContent.trim()) {
        const text = element.textContent.trim();
        // Only add if it's not already included and it's not just whitespace
  ...
  // Method 3: If still no text, try to get the raw text content
  if (!allText.trim()) {
...
export function convertParsedSlidesToAppSlidesBasic(parsedSlides: ParsedSlide[], projectId: string): Slide[] {
